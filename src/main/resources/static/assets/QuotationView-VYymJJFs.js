import { D as createCommentVNode, Dt as toDisplayString, E as createBlock, K as withCtx, O as createElementBlock, S as Fragment, T as createBaseVNode, Tt as normalizeClass, V as openBlock, W as resolveComponent, a as carImageUrl, c as formatPrice, i as carApi, j as createTextVNode, m as quotationApi, o as cartApi, ot as unref, q as withDirectives, tt as ref, v as useDefaultCarImage, w as computed, z as onMounted } from "./api-BIOmrm3q.js";
import { a as useRoute, d as vModelText, i as useCartStore, n as useAuthStore, o as useRouter, t as _plugin_vue_export_helper_default } from "./index-B5QpE601.js";
	...
	setup(__props) {
		...
		async function convertToOrder() {
			...
			try {
				...
				if (!data?.success || !data.data) throw new Error(data?.message || "Không thể tạo đơn hàng");
				notifyDataUpdated();
				router.push(`/order/detail/${data.data.id}`);
			} catch (e) {
				error.value = e.response?.data?.message || e.message || "Không thể tạo đơn hàng";
			} finally {
				submitting.value = false;
			}
		}
