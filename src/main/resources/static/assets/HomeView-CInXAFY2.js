	...
	setup(__props) {
		...
		async function withActivePromotion(car) {
			try {
				...
				const discountAmount = promotion.type === "PERCENT" ? price * Number(promotion.value || 0) / 100 : Number(promotion.value || 0);
		...
		async function addToCart(id) {
			...
			if (data.success) {
				...
				alert.value = "";
			} else alert.value = data.message || "Không thể thêm vào phiếu đặt cọc xe";
		}
		return (_ctx, _cache) => {
			...
			return openBlock(), createElementBlock("div", _hoisted_1, [
				...
				createBaseVNode("section", _hoisted_3, [createBaseVNode("div", _hoisted_4, [
					...
					createBaseVNode("div", _hoisted_5, [createVNode(_component_router_link, {
						...
						default: withCtx(() => [..._cache[1] || (_cache[1] = [createTextVNode("Phiếu đặt cọc xe của bạn", -1)])]),
						_: 1
					})])
