<template>
  <div class="container cs-container py-5">
    <div class="cs-card profile-card overflow-hidden">
      <div class="profile-header p-4">
        <div class="avatar">{{ profile?.fullname?.charAt(0) || profile?.username?.charAt(0) || 'U' }}</div>
        <div>
          <h2 class="cs-page-title mb-1">Hồ sơ cá nhân</h2>
          <p class="mb-0 text-secondary">Quản lý thông tin và bảo mật tài khoản</p>
        </div>
      </div>
      <div class="p-4">
        <div v-if="profile">
          <form class="vstack gap-3" @submit.prevent="updateProfile">
            <div><label class="form-label">Username</label><input :value="profile.username" class="form-control"
                disabled /></div>
