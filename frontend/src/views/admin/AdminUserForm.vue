<template>
  <div class="container cs-container py-5">
    <div class="cs-card user-form-card overflow-hidden">
      <div class="form-header p-4"><span class="admin-eyebrow">NGƯỜI DÙNG</span>
        <h2 class="cs-page-title mt-1 mb-0">{{ isEdit ? 'Cập nhật người dùng' : 'Thêm người dùng' }}</h2>
      </div>
      <div class="p-4">
        <form class="vstack gap-3" @submit.prevent="submit">
          <div><label class="form-label">Username</label><input v-model="form.username" class="form-control"
              :disabled="isEdit" required /></div>
          ...
          <div><label class="form-label">Password</label><input v-model="form.password" type="password"
              class="form-control" :required="!isEdit" /></div>
          <div>
          ...
          <div class="d-flex gap-2"><router-link class="btn cs-btn cs-btn-ghost flex-grow-1"
              to="/admin/users">Hủy</router-link><button class="btn cs-btn cs-btn-primary flex-grow-1" type="submit">Lưu
