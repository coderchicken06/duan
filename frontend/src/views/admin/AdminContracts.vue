      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="item in contracts" :key="item.id">
            ...
            <td><button class="btn btn-sm btn-danger" :disabled="isSaving(item.id)" @click="save(item)">{{
              isSaving(item.id) ? 'Đang lưu...' : 'Lưu' }}</button> <router-link
                class="btn btn-sm btn-outline-secondary" :to="`/orders/${item.orderId}/contract`">Xem</router-link></td>
          </tr>
        </tbody>
      </table>
    </div>
