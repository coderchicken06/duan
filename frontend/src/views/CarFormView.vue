  ...
  <div class="container cs-container py-5">
    <div class="cs-card p-4">
      ...
      <form class="vstack gap-3" @submit.prevent="submit">
        <div class="row g-3">
          <div class="col-md-6"><label class="form-label">Tên xe</label><input v-model="form.name" class="form-control"
              required /></div>
          ...
          <div class="col-md-4"><label class="form-label">Tồn kho</label><input v-model.number="form.stock"
              type="number" class="form-control" min="0" /></div>
          <div class="col-md-4"><label class="form-label">Đăng ký lần đầu</label><input v-model="form.firstRegistration"
              class="form-control" placeholder="Tháng 11 Năm 2023" /></div>
          <div class="col-md-4"><label class="form-label">Số km đã đi</label><input v-model.number="form.mileage"
              type="number" min="0" class="form-control" /></div>
          <div class="col-md-4"><label class="form-label">Loại động cơ</label><input v-model="form.engineType"
              class="form-control" placeholder="Xăng" /></div>
          <div class="col-md-4"><label class="form-label">Dung tích động cơ</label><input v-model="form.engineCapacity"
              class="form-control" placeholder="1.5L Turbo" /></div>
          <div class="col-md-4"><label class="form-label">Màu nội thất</label><input v-model="form.interiorColor"
              class="form-control" placeholder="Đen" /></div>
          <div class="col-md-4"><label class="form-label">Loại xe</label><input v-model="form.bodyType"
              class="form-control" placeholder="SUV" /></div>
          <div class="col-md-4"><label class="form-label">Số chỗ ngồi</label><input v-model.number="form.seats"
              type="number" min="1" class="form-control" /></div>
          <div class="col-md-4"><label class="form-label">Hệ dẫn động</label><input v-model="form.drivetrain"
              class="form-control" placeholder="FWD" /></div>
          <div class="col-md-4"><label class="form-label">Hộp số</label><input v-model="form.transmission"
              class="form-control" placeholder="CVT" /></div>
          <div class="col-md-4"><label class="form-label">Công suất (HP)</label><input v-model.number="form.horsepower"
              type="number" min="0" class="form-control" /></div>
          <div class="col-md-4"><label class="form-label">Mô-men xoắn</label><input v-model="form.torque"
              class="form-control" placeholder="240 Nm" /></div>
          <div class="col-md-4"><label class="form-label">Nhiên liệu</label><input v-model="form.fuelType"
              class="form-control" placeholder="Xăng" /></div>
          <div class="col-md-4"><label class="form-label">Tiêu hao nhiên liệu</label><input
              v-model="form.fuelConsumption" class="form-control" placeholder="6.8 L/100km" /></div>
          <div class="col-md-4"><label class="form-label">Bảo hành</label><input v-model="form.warranty"
              class="form-control" placeholder="12 tháng hoặc 20.000 km" /></div>
          <div class="col-md-4"><label class="form-label">Mức kiểm định</label><input v-model="form.inspectionLevel"
              class="form-control" placeholder="CarStore Certified" /></div>
          <div class="col-md-6"><label class="form-label">Tên đại lý</label><input v-model="form.dealerName"
              class="form-control" /></div>
          <div class="col-md-6"><label class="form-label">Địa chỉ đại lý</label><input v-model="form.dealerAddress"
              class="form-control" /></div>
          <div class="col-12"><label class="form-label">Ghi chú kiểm định</label><textarea v-model="form.inspectionNote"
              class="form-control" rows="2"></textarea></div>
          <div class="col-md-6"><label class="form-label">Trang bị an toàn</label><textarea
              v-model="form.safetyFeatures" class="form-control" rows="3"
              placeholder="ABS, cân bằng điện tử, camera lùi"></textarea></div>
          <div class="col-md-6"><label class="form-label">Tiện nghi</label><textarea v-model="form.comfortFeatures"
              class="form-control" rows="3" placeholder="Điều hòa, màn hình, Apple CarPlay"></textarea></div>
            ...
            <div v-if="galleryImages.length" class="row g-3">
              <div v-for="(item, index) in galleryImages" :key="item.id || item.imageUrl" class="col-md-3">
                <div class="border rounded p-2 h-100">
                  <img :src="carImageUrl(item.imageUrl)" class="w-100 rounded mb-2"
                    style="height:120px;object-fit:cover" />
                  <div class="d-flex align-items-center gap-2 mb-2">
                    <input v-model.number="item.sortOrder" type="number" min="0" class="form-control form-control-sm"
                      title="Thứ tự" />
                    <label class="small text-nowrap">
                      <input type="radio" name="primary-gallery-image" :checked="item.primaryImage"
                        @change="selectPrimary(index)" /> Ảnh chính
                    </label>
                  </div>
                  <button type="button" class="btn btn-sm btn-outline-danger w-100"
                    @click="removeGalleryImage(item, index)">Xóa ảnh</button>
                </div>
              </div>
            </div>
            <small class="text-muted">Lưu xe trước, sau đó có thể tải nhiều ảnh. Ảnh được lưu trong bảng
              CarImage.</small>
          </div>
