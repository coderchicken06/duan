<template>
  <main class="contract-page py-5">
    <div v-if="loading" class="container text-center py-5"><span class="spinner-border text-danger"></span>
      <p class="mt-3">Đang tải hợp đồng...</p>
    </div>
    ...
    <article v-else class="contract-sheet container">
      <header class="contract-header">
        ...
        <div class="text-center">
          ...
          <p>Số: {{ contract.contractNo || `HĐ-${contract.id}-${order.id}` }}</p><small v-if="contract.quotationId">Báo
            giá #{{ contract.quotationId }}</small>
        </div>
      ...
      <section class="contract-section">
        ...
        <p class="mb-0"><span class="text-secondary">Nhân viên phụ trách: </span><strong>{{ contract.employeeUsername ||
            'Đang chờ phân công' }}</strong></p>
      </section>
      ...
      <section class="contract-section">
        ...
        <div v-for="item in details" :key="item.id" class="vehicle-row">
          ...
          <div>
            ...
            <strong>{{ formatPrice(item.price) }} VNĐ × {{ item.quantity }}</strong>
          </div>
        </div>
      </section>
      ...
      <section class="contract-section">
        ...
        <div class="info-grid">
          ...
          <p><span>Trạng thái cọc</span><b class="status-badge" :class="{ paid: contract.depositStatus === 'PAID' }">{{
            contract.depositStatus }}</b></p>
      ...
      <section class="contract-section payment-summary">
        ...
        <p><span>Tiền đặt cọc</span><strong>{{ formatPrice(contract.depositAmount || contract.deposit) }} VNĐ</strong>
        </p>
      ...
      <section class="contract-section terms">
        <h2>Điều khoản</h2>
        <ol>
          <li>Xe được bàn giao theo đúng thông tin và tình trạng hai bên đã xác nhận.</li>
          <li>Chính sách bảo hành áp dụng theo thông tin công bố của showroom và nhà sản xuất.</li>
          <li>Thời gian giao xe được thống nhất sau khi hoàn tất nghĩa vụ thanh toán.</li>
          <li>Việc hoàn tiền cọc thực hiện theo trạng thái đơn và thỏa thuận được hai bên xác nhận.</li>
        </ol>
      </section>
      <section class="signatures">
        <div>KHÁCH HÀNG<small>(Ký và ghi rõ họ tên)</small></div>
        <div>NHÂN VIÊN<small>(Ký và ghi rõ họ tên)</small></div>
        <div>ĐẠI DIỆN SHOWROOM<small>(Ký, đóng dấu)</small></div>
      </section>
...
.contract-sheet {
  ...
  background: #fff;
  padding: 42px;
  box-shadow: 0 12px 35px #1f29371a
}
