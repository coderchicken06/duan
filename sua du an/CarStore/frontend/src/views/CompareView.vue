<template>
  <main class="container cs-container py-5">
    <div class="compare-heading"><div><span class="eyebrow">CARSTORE SELECT</span><h1>So sánh xe</h1><p>Đối chiếu tối đa 3 xe theo giá, vận hành, tiện nghi và tình trạng kiểm định.</p></div><button class="ford-btn-outline" @click="clearAndBack">Xóa lựa chọn</button></div>
...
onMounted(async()=>{
  const ids=String(route.query.ids||selectedIds.value.join(','))
    .split(',')
    .map(Number)
  ...
  try {
    const responses=await Promise.all(ids.map(id=>carApi.getById(id)))
    cars.value=responses.map(r=>r.data?.data||r.data).filter(Boolean)
  } catch (error) {
